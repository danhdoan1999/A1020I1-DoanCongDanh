...
public class WebSercurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests().antMatchers("/","/home", "/login", "/logout").permitAll()
                .and()
                // trang này dành cho user và admin có role quản lí ,  giám đốc , nhân viên
                .authorizeRequests().antMatchers("/customer**","/service**","/contract**").access("hasAnyRole('ROLE_QUANLY', 'ROLE_GIAMDOC', 'ROLE_NHANVIEN')");
                //.and()
